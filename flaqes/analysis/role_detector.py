...
def _detect_junction_signals(table: Table, graph: SchemaGraph) -> list[Signal]:
    ...
    if table.foreign_keys:
        ...
        non_audit_cols = [
            c for c in non_fk_cols if c.name not in {"created_at", "updated_at", "id"}
        ]
...
def _detect_event_signals(table: Table, graph: SchemaGraph) -> list[Signal]:
    ...
    has_created = any(
        n in col_names for n in {"created_at", "created", "timestamp", "event_time"}
    )
    has_updated = any(
        n in col_names for n in {"updated_at", "updated", "modified_at", "modified"}
    )
...
def _detect_scd_type2_signals(table: Table, graph: SchemaGraph) -> list[Signal]:
    ...
    has_valid_from = any(
        n in col_names
        for n in {"valid_from", "effective_from", "start_date", "from_date"}
    )
    has_valid_to = any(
        n in col_names for n in {"valid_to", "effective_to", "end_date", "to_date"}
    )
    ...
    has_is_current = any(
        n in col_names for n in {"is_current", "is_active", "current_flag"}
    )
    if has_is_current:
        signals.append(
            Signal(
...
class RoleDetector:
    ...
    def detect_all(self, graph: SchemaGraph) -> dict[str, TableRoleResult]:
        ...
        """
        return {table.fqn: self.detect(table, graph) for table in graph}
    ...
    def detect_with_validation(
        ...
        if result.primary_role == RoleType.FACT:
            ...
            dim_count = sum(
                1
                for t in referenced
                if self.detect(t, graph).primary_role
                in (RoleType.DIMENSION, RoleType.LOOKUP)
            )
