...
def _detect_wide_table(table: Table, intent: Intent | None) -> DesignTension | None:
    ...
    severity = (
        Severity.CRITICAL if column_count >= critical_threshold else Severity.WARNING
    )
...
def _detect_nullable_foreign_key(
    table: Table, intent: Intent | None
) -> list[DesignTension]:
...
def _detect_jsonb_overuse(table: Table, intent: Intent | None) -> DesignTension | None:
    ...
    """
    json_columns = [
        c for c in table.columns if c.data_type.category == DataTypeCategory.JSON
    ]
...
def _detect_missing_audit_columns(
    table: Table, intent: Intent | None
) -> DesignTension | None:
    ...
    has_created = any(
        n in col_names for n in {"created_at", "created_date", "create_time"}
    )
    has_updated = any(
        n in col_names for n in {"updated_at", "modified_at", "update_time"}
    )
...
def _detect_text_without_constraint(
    table: Table, intent: Intent | None
) -> list[DesignTension]:
            ...
            if raw == "text" or raw == "character varying":
                tensions.append(
                    DesignTension(
                        ...
                        alternatives=[
                            Alternative(
                                description="Add CHECK constraint for reasonable length",
...
def _detect_denormalization(
    table: Table, graph: SchemaGraph, intent: Intent | None
) -> list[DesignTension]:
...
class TensionAnalyzer:
    ...
    def analyze_table(self, table: Table, graph: SchemaGraph) -> list[DesignTension]:
        ...
        filtered = [
            t for t in tensions if severity_order.index(t.severity) >= min_index
        ]
