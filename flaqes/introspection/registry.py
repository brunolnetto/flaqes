...
def register_introspector(
    engine: str,
) -> Callable[[type[Introspector]], type[Introspector]]:
    """
    ...
    if engine == "postgresql":
        try:  # pragma: no cover
            ...
            raise ImportError(
                ...
                f"Install with: pip install flaqes[postgresql]\n"
                f"Original error: {e}"
            ) from e
    elif engine == "mysql":  # pragma: no cover
        raise NotImplementedError(
            "MySQL support is not yet implemented. Contributions welcome!"
        )
    elif engine == "sqlite":  # pragma: no cover
        raise NotImplementedError(
            "SQLite support is not yet implemented. Contributions welcome!"
        )
...
def get_introspector(engine: str, dsn: str) -> Introspector:
    ...
    if engine not in _registry:
        ...
        raise ValueError(
            f"Unsupported database engine: '{engine}'. Supported engines: {supported}"
        )
